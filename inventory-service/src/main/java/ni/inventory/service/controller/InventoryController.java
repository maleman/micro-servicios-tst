...
public class InventoryController {
	...
	public ResponseEntity<String> in(@PathVariable Long wharehouse_id,
	...
	@PutMapping("/out/{wharehouse_id}/{product_id}/{qty}")
	public ResponseEntity<String> out(@PathVariable Long wharehouse_id,
		...
		entity.setQtyOnHand(entity.getQtyOnHand().subtract(qty));
		var newEntity = productInventoryRepository.save(entity);
		return ResponseEntity.ok("200");
	}
	
	
	@PutMapping("/reserve/{wharehouse_id}/{product_id}/{qty}")
	public ResponseEntity<String> reserve(@PathVariable Long wharehouse_id,
			@PathVariable Long product_id, @PathVariable BigDecimal qty){
		...
		if(entity.getQtyOnHand().compareTo(qty) < 0)
			 return ResponseEntity
		                .status(HttpStatus.FORBIDDEN)
		                .body("Error Message");
		
		entity.setQtyOnHand(entity.getQtyOnHand().subtract(qty));
		entity.setQtyReserved(qty);
		var newEntity = productInventoryRepository.save(entity);
		return ResponseEntity.ok("200");
	}
}
